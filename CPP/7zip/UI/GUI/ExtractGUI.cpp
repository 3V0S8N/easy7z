...
// Easy7zip: error-state flag — defined in ProgressDialog2.cpp, set there when
// extraction/copy errors occur. Read here to gate post-extract actions.
  ...
  */
  ...
  if (extracter.ThreadFinishedOK && !g_bProcessError)
  {
    ...
    if (deleteSourceFile)
    {
      ...
      {
        UString strFilePath = archivePathsFull[i];
