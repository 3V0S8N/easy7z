...
static BOOL IsDirectory(const wchar_t * lpszPathFile)
{
...
void CExtractDialog::OnButtonOpenPath()
{
  ...
  else
  {
    ...
    MessageBoxW((HWND)*this, szMsg, L"7-Zip", MB_ICONEXCLAMATION);
  }
}
...
{
  if (n64Byte < 1000ui64)
    return swprintf(lpszBuf, ccBuf, L"%I64d B", n64Byte);
  if (n64Byte < 1024000ui64)
    return swprintf(lpszBuf, ccBuf, L"%.1f KB", (double)n64Byte / 1024.0);
  if (n64Byte < 1048576000ui64)
    return swprintf(lpszBuf, ccBuf, L"%.2f MB", (double)n64Byte / 1048576.0);
  if (n64Byte < 1073741824000ui64)
    return swprintf(lpszBuf, ccBuf, L"%.2f GB", (double)n64Byte / 1073741824.0);
  return swprintf(lpszBuf, ccBuf, L"%.2f TB", (double)n64Byte / 1099511627776.0);
}
